### Making a  nostalgic configuration NetBSD in virt-manager ###
...
I used `NetBSD-10.0-amd64.iso` and installed it in virt-manager (QEMU/KVM)
...
.... netbsd1.png
<p align="center">
<img  src="https://github.com/nevillejackson/Unix/blob/main/netbsd/netbsd1.png?raw
=true">
</p>
...
That is an `sh` shell prompt. The other options are `csh` and `zsh`.  That is right, no `bash` although `sh` is a good approximation.  For a real Berkekely experienc I should be using 'csh' as the interactive shell.

Now there is an X window system, called CTWM, which is a modified TWM. You can start it with `startx`

.....  netbsd4.png

So you get a terminal window ( a uxterm with no scrollbar) and there are 4 workspaces ( lower right). Here is a menu that comes with a right click on background.
...
I chose the `virti0` device model for the NIC . The default `e1000` did not work with NetBSD for me.
...
....  netbsd9.png
 ...
 To make it start at boot I have to mmake some settings in `/etc/rc.conf`
```
ifconfig_vioif0="up"
dhcpcd=YES
```
...
```
I dont recommend it. 
Learn fom my oversight.... when you install NetBSD configure the network at install time. 
...
FFS is basis of Linux ext filesystems. Linux can read files from an FFS filesystem if it has kernel modules supporting UFS.
...
#### The package system ####
The original Berkekey Unix did not have a package system. You got what came with the distribution. There was internet, but it only supported email and ftp. So oe could download source code ( Fortran or C usually) from  sites like Netlib, and compile it , like in Gentoo today only without the help of portage.
...
Version control seems to ba a matter of setting the PKG_PATH environment variable.
...
Most distros provide a procedure for dealing with source code packages in a standardised way. In NetBSD the build utility for souce code is called `pkgsrc`. 

https://www.netbsd.org/docs/software/packages.html

It supports most BSD distros, and Linux, ( ie you can use pkgsrc in distrox other than NetBSD). It is different to the Gentoo portage system or the FreeBSD ports system.
...
Lots ouf output, including some questions aout overwriting config files.
...
Well it has page faults but no 'pi' or 'po'.  So it is accessing pages already in menmory, but not swapping them to disk.  It has a lot of free memory. I thought compiling would take more than that?
...
I might try and do a look at modern NetBSD, but that is another topic.
