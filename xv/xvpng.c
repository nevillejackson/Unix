...
*/
  ...
  */
  ...
  height = h;
/*
  png_set_IHDR(png_ptr,info_ptr,width,height,NULL,NULL,NULL,NULL,NULL);
*/
  if (w <= 0 || h <= 0) {
...
/*
      info_ptr->colorType = PNG_COLOR_TYPE_RGB;
*/
...
*/
      ...
      linesize = w;
/*
      info_ptr->colorType = PNG_COLOR_TYPE_PALETTE;
*/
...
*/
  ...
  png_set_IHDR(png_ptr, info_ptr, w, h, bit_depth, colorType, interlace_type, compression_type, filter_type);

  png_set_PLTE(png_ptr, info_ptr, palette, num_palette);

  png_set_gAMA(png_ptr, info_ptr, vgamma);
...
*/
  ...
  sprintf(pinfo->fullInfo + strlen(pinfo->fullInfo),
          ", %sinterlaced. (%d bytes)", 
          interlace_type ? "" : "non-", filesize);

/*
  sprintf(pinfo->shrtInfo, "%lux%lu PNG", info_ptr->width, info_ptr->height);
*/
  sprintf(pinfo->shrtInfo, "%lux%lu PNG", width, height);
...
/*
    ...
    if ((colorType == PNG_COLOR_TYPE_GRAY_ALPHA ||
         (colorType == PNG_COLOR_TYPE_GRAY && HAVE_tRNS)) &&
        (imagebgR != imagebgG || imagebgR != imagebgB))  /* i.e., colored bg */
    {
